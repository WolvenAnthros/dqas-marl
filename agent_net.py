...
class Agent_net(object):
    ...
    def choose_action(self, obs_n, last_onehot_a_n, avail_a_n, epsilon):
        with torch.no_grad():
            # a_n = [np.random.choice(np.nonzero(avail_a)[0]) for avail_a in avail_a_n]  # uncomment to compare with pure random
            if np.random.uniform() < epsilon:  # epsilon-greedy
                ...
                # print(f'Allowed actions: {avail_a_n}')
                # print(f'Q value: {q_value}')
                # print(f'Observation is {obs_n}')
                # print(f'Actions: {a_n}')
            return a_n
    ...
    def train(self, replay_buffer, total_steps):
        ...
        self.aim_run.track(loss, name='loss')
